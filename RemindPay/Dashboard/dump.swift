...
final class GymBreakView: UIView {
    ...
    private func setupUserDataContainer() {
        let container = UIView()
        containerView.addSubview(container)
        userDataContainer = container
        container.setTranslatesMask()
        let leading = container.leadingAnchor.constraint(equalTo: titleLabel.leadingAnchor)
        let trailng = container.trailingAnchor.constraint(equalTo: containerView.trailingAnchor)
        let top = container.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 12)
        NSLayoutConstraint.activate([leading, trailng, top])
    }

    private func setupActiveData() {
        let label = getLabel(placeholder: "Active Users", text: "2000")
        activeLabel = label
        userDataContainer.addSubview(label)
        label.setTranslatesMask()
        let leading = label.leadingAnchor.constraint(equalTo: userDataContainer.leadingAnchor)
        let top = label.topAnchor.constraint(equalTo: userDataContainer.topAnchor)
        NSLayoutConstraint.activate([leading, top])
    }
